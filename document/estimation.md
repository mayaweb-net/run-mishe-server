...
ابزار گلوگاه **یک عدد تعادل بین CPU و GPU** است — بدون بازی و بدون جدول رزولوشن.
نمونه‌های NotebookCheck معمولاً روی یک CPU ثابت‌اند؛ پروفایل کالیبره‌شده‌ی بازی
عمداً استفاده نمی‌شود.

از `gamingIndex` دو قطعه با ضرایب cold-start `MEDIUM` سقف نسبی ساخته می‌شود:
...
اگر MAPE از ۴۰٪ بدتر شد، نتیجه را ننویس (داده احتمالاً خراب است). اگر از cold-start
کمی بدتر بود ولی زیر سقف ۴۰٪ ماند، باز هم بنویس — اولویت با استفاده از `FpsSample` است.
قبل از انتشار محصول می‌توانی دوباره شرط «بهتر از cold-start» را سفت‌تر کنی.
